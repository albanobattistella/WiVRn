...
video_encoder_vulkan::video_encoder_vulkan(wivrn_vk_bundle & vk, vk::Rect2D rect, vk::VideoEncodeCapabilitiesKHR in_encode_caps, float fps, uint64_t bitrate) :
        xrt::drivers::wivrn::VideoEncoder(true), vk(vk), rect(rect), encode_caps(patch_capabilities(in_encode_caps)), fps(fps)
{
...
std::optional<xrt::drivers::wivrn::VideoEncoder::data> video_encoder_vulkan::encode(bool idr, std::chrono::steady_clock::time_point target_timestamp, uint8_t encode_slot)
{
	if (idr)
		send_idr_data();
...
void video_encoder_vulkan::present_image(bool idr, vk::Image src_yuv, vk::raii::CommandBuffer & command_buffer, vk::Fence fence, uint8_t encode_slot)
{
